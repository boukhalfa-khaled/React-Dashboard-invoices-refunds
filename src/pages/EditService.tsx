...
const EditService: React.FC = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axiosPrivate.put(`${SERVICES_URL}${id}`, {
        title,
        description,
        price,
        stock,
      ...
      let ServiceTitle = response.data.title;
      setSuccessMsg(`Service ${ServiceTitle} updated successfully`)
    } catch (err) {
      if(!err?.response){
        setErrMsg('Server Problem')
      } else if (err.response?.status === 400) {
        setErrMsg('please enter a valid inputs')
      } else {
  ...
  useEffect(() => {
    ...
    const getService= async () => {
        try {
            const response = await axiosPrivate.get(`${SERVICES_URL}${id}`, {
