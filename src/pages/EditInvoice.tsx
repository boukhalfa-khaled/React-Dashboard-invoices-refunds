...
const EditInvoice: React.FC = () => {
  ...
  const calculateTotal = (price, qty) => {
    ...
    if (!isNaN(parsedPrice) && !isNaN(parsedQty)) {
      const newTotal = parsedPrice * parsedQty;
      setTotal(newTotal);
    } else {
      setTotal('00');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosPrivate.put(`${INVOICES_URL}${id}`, {
        title ,
        description ,
        price, 
        qty,
      ...
      setSuccessMsg(`Invoice ${InvoiceTitle} updated successfully`)
    } catch (err) {
      if(!err?.response){
  ...
  useEffect(() => {
    ...
    const getService= async () => {
        try {
            const response = await axiosPrivate.get(`${INVOICES_URL}${id}`, {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Edit An Invoice" />
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        calculateTotal(e.target.value, qty);
                      }}
                      value={price}
                      required
                      placeholder="Re-enter your password"
