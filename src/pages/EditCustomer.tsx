...
const EditCustomer: React.FC = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("islam")
      const response = await axiosPrivate.put(`${CUSTOMER_URL}${id}`, {
        email ,
        name ,
        phone, 
        address,
      ...
      setSuccessMsg(`customer ${customerName} updated successfully`)
    } catch (err) {
      if(!err?.response){
  ...
  useEffect(() => {
    ...
    const getCustomer = async () => {
        try {
            const response = await axiosPrivate.get(`${CUSTOMER_URL}${id}`, {
