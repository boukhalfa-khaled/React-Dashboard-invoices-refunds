  ...
  useEffect(() => {
    ...
    const getProfile = async () => {
        try {
            ...
            let  cutomerss = Array.from(response.data);
            setCustomers(cutomerss);
        } catch (err) {
          if(!err?.response){
...
const handleDelete = async (id, CustomerName) => {
  try {
    const response = await axiosPrivate.delete(`${CUSTOMERS_URL}/${id}`, 
    {
     ...
     if (response.status === 204) {
      setSuccessMsg(`Customer ${CustomerName} deleted successfully.`);
    } else {
          ...
          <tbody>
            {customers.map((customer, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">{customer.date}</p>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
